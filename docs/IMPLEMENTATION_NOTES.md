...
- LLM analysis: `llm/analyzer.py` validates JSON output into Pydantic `AISignal` and rejects unknown `evidence_id` references.
...
- The LLM provider client is not implemented; current runtime uses disabled analysis unless a caller injects a completion function into `StructuredLLMAnalyzer`.
