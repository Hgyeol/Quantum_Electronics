...
DATA = [
    ...
    ("그래프 보기", "유사 패턴 찾기", "지금 차트와 모양이 비슷했던 과거 사례를 다른 종목에서 찾아주고, 그때 이후에 얼마나 올랐는지/내렸는지를 알려줍니다.", "유사 패턴 검색 카드", "2026-06-09"),
...
DEV_DATA = [
    ...
    ("5-5. 차트 패턴 유사 사례 검색", "5-5-1-1. 고른 구간 종가를 z-정규화한다.\n5-5-1-2. 전 종목 슬라이딩 윈도우를 유클리드 거리로 1차 필터링한다.\n5-5-1-3. 상위 후보를 DTW(Sakoe-Chiba 밴드)로 재정렬한다.\n5-5-1-4. 각 사례의 이후 N일 수익률을 집계해 통계(평균·중앙값·상승비율)를 낸다.", "종가 시계열, 유사도, 이후 수익률", "SimilarCase", "유사 패턴 검색 카드", "chart.pattern_match"),

    ("6. 다중 조건 검색식",),
    ("6-1. 조건 선택", "6-1-1-1. 28종 조건을 카테고리별 칩으로 다중 선택한다.", "조건 목록", "ScreenerCondition", "스크리너 조건 선택 화면", "web"),
    ("6-2. 조건 파라미터 설정", "6-2-1-1. 조건별 임계값(배수·일수·기간 등)을 칩 팝오버로 조정한다.", "임계값, 일수, 기간", "ScreenerParams", "조건 파라미터 팝오버", "web"),
    ("6-3. 조건 설명 제공", "6-3-1-1. 각 조건 칩의 ⓘ 버튼으로 쉬운 말 설명 팝오버를 표시한다.\n6-3-1-2. 설명 텍스트는 조건 정의에 정적으로 포함된다.", "조건 설명 텍스트", "ConditionItem", "조건 설명 팝오버", "web"),
    ("6-4. DB 기반 조건 평가", "6-4-1-1. 사전 수집된 일봉·투자자 데이터로 전체 종목을 검사한다.\n6-4-1-2. 거래량 급등·골든크로스·정배열·MACD·OBV 등 25종 조건을 계산한다.", "일봉 OHLCV, 투자자 데이터", "ScreenerResult", "스크리너 결과 목록", "services.screener_conditions"),
    ("6-5. 실시간 조건 평가", "6-5-1-1. 체결강도·신고가 근접·상한가 포착 3종은 KIS API로 실시간 조회한다.", "체결강도, 신고가, 상한가", "RankItem", "스크리너 결과 목록", "services.ranking"),
    ("6-6. 야간 데이터 수집 배치", "6-6-1-1. 전 종목 일봉 OHLCV와 투자자 데이터를 매일 수집해 SQLite에 저장한다.", "일봉 OHLCV, 투자자 데이터", "ScreenerDB", "", "services.screener_collector"),

    ("7. 시장현황 랭킹",),
    ("7-1. 거래량/거래대금 순위", "7-1-1-1. 거래량·거래대금 상위 종목을 조회한다.", "거래량, 거래대금 순위", "RankItem", "시장현황 랭킹 화면", "services.ranking"),
    ("7-2. 급등주 순위", "7-2-1-1. 등락률 상위 종목을 양수 등락률만 필터링해 조회한다.", "등락률 순위", "RankItem", "시장현황 랭킹 화면", "services.ranking"),
    ("7-3. 외국인/기관 순매수 순위", "7-3-1-1. 외국인·기관 순매수 상위 종목을 조회한다.", "순매수량 순위", "RankItem", "시장현황 랭킹 화면", "services.ranking"),
    ("7-4. 자동 갱신", "7-4-1-1. 실시간 탭은 15초마다 데이터를 갱신한다.", "순위 목록", "RankItem", "시장현황 랭킹 화면", "web"),

    ("8. 종목 탐색 · 화면 흐름",),
    ("8-1. 종목 검색", "8-1-1-1. 종목명·코드로 검색해 상세 페이지로 이동한다.", "종목명, 종목코드", "StockMaster", "헤더 검색창", "services.outlook"),
    ("8-2. 호버 미리보기", "8-2-1-1. 목록에서 종목 호버 시 미니 차트·한줄요약·시세를 우측 패널에 표시한다.", "미니 차트, 한줄요약, 시세", "MarketQuote", "우측 미리보기 패널", "web"),
    ("8-3. URL 라우팅 · 뒤로가기 복원", "8-3-1-1. 종목 진입 시 URL에 종목코드를 반영한다.\n8-3-1-2. 뒤로가기 시 직전 탭·스크롤 위치·스크리너 조건을 복원한다.", "종목코드, 화면 상태", "HistoryState", "", "web"),
    ("8-4. 종목 로고 표시", "8-4-1-1. 종목별 로고 이미지를 CDN에서 불러와 표시한다.", "로고 이미지", "StockLogo", "종목 목록·상세", "web"),

    ("9. 공통 · 환경",),
    ("9-1. 뉴스 수집", "9-1-1-1. 종목 관련 뉴스를 수집해 evidence로 만든다.", "뉴스 기사", "Evidence", "", "news"),
    ("9-2. 공시 수집", "9-2-1-1. DART 등에서 공시를 수집해 evidence로 만든다.", "공시 항목", "Evidence", "", "disclosure"),
    ("9-3. 큰 글씨 모드", "9-3-1-1. 토글로 전체 화면을 1.3배 확대한다.", "확대 배율", "ThemeState", "큰 글씨 토글", "web"),
    ("9-4. KIS 인증", "9-4-1-1. KIS Open API 접근 토큰과 WebSocket approval_key를 발급·관리한다.", "접근 토큰, approval_key", "KISAuth", "", "services"),
]
